        ...
        with tab3:
            ...
            if not intros_df.empty:
                ...
                if 'intro' not in intros_df.columns:
                    ...
                    # 添加搜索功能
                    search_term = st.text_input("🔍 搜索植物名称", help="输入植物名称查看详细介绍")
                    ...
                    for idx, row in intros_df.iterrows():
                        ...
                        # 添加搜索匹配逻辑
                        is_match = search_term.lower() in plant_name.lower() if search_term else False
                        
                        with cols[idx % 3]:
                            # 根据匹配状态设置默认展开
                            with st.expander(f"🌴 {plant_name}", expanded=is_match):  # 修改这里
                                try:
                                    # 显示植物图片
                                    img_path = pathlib.Path(__file__).parent / "plants" / f"{plant_name}.jpg"
                                ...
                                # 显示特征标签
                                if '好养护' in row:
                                    st.markdown(f"**养护难度:** {'★' * int(row['好养护'])}")
                                if '适合新手' in row:
                                    st.markdown(f"**新手推荐:** {'✅ 适合' if row['适合新手'] else '❌ 不适合'}")
                                ...
                                if is_match:
                                    ...
                                    st.markdown("""
                                             <script>
                                            window.parent.document.querySelector('div[data-testid="stExpander"]:has(.stSuccess)').scrollIntoView();
                                            </script>
                                            """, unsafe_allow_html=True)
            else:
                st.warning("植物介绍数据暂未加载，请检查plant_intros.csv文件")
